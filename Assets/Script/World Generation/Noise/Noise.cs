...
*/
    ...
    */
    ...
    public static float Get3DNoise(float oX, float oY, float oZ, float period)
    {
        float x = oX * period;
        float y = oY * period;
        float z = oZ * period;
    ...
    public static Vector3 Perlin3D(float period, float amplitude, Vector3 vect, Vector3 origin)
    {
        Vector3 normal = (origin - vect).normalized;

        float noise = Get3DNoise(vect.x, vect.y, vect.z, period);
        return vect + ((normal * noise) * amplitude);
